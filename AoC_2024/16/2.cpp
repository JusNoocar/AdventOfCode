...
void FillSofa(int v, std::vector<std::vector<int>>& sofa, std::vector<std::vector<std::pair<int, int>>>& g, std::vector<int>& dist, std::vector<int>& used) {
  used[v] = true;
  int wid = (int)(v / 4) % n;
  int len = (int)(v / 4) / n;
  ...
  sofa[len][wid] = 1;
  for (auto [w, u] : g[v]) {
    if (!used[u] && dist[u] + w == dist[v]) {
      FillSofa(u, sofa, g, dist, used);
    }
  }
}
...
int main() {
  ...
  std::vector<int> dist = Dijkstra(start, g);
  std::cout << "ok";


  int ans = INF;
  std::vector<int> ans_end;
  for (int end1 : ends) {
    if (dist[end1] <= ans) {
      if (dist[end1] < ans) {
          ans_end.clear();
      }
      ans_end.push_back(end1);
      ans = dist[end1];
    }
  }
  std::cout << ans << "\n";
  std::vector<std::vector<int>> sofa(n, std::vector<int>(n, 0));
  std::vector<int> used(n * n * 4, false);
  for (int end1: ans_end) {
      FillSofa(end1, sofa, g, dist, used);
  }
